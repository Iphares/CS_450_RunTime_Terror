...
************************************************************************************/
		...
		char temp[20];
		int i,j;
		for(alias_init = 0; alias_init < 20; alias_init++)	{
			memset(&temp[0], '\0', 20);
			for(i = 0; i<strlen(FirstToken); i++){
				temp[i] = FirstToken[i];
			}
			if(strcmp(temp, FirstToken) == 0)	{

				for(j = 0; j<=strlen(FirstToken); j++){
					FirstToken[j] = Aliased[alias_init][j];
					if(j == strlen(FirstToken)){
						FirstToken[j]=NULL;
					}
				}
				...
				if(strcmp(FirstToken,"alias") == 0 && strcmp(FourthToken,NULL) == 0)		{
					int i, j;
					for(i=0; i<20; i++){
						if(Aliasing[i][0] == NULL){
							break;
						}
					}
					for(j=0; j<20; j++){
						if(ThirdToken[j] != NULL){
							Aliasing[i][j] = ThirdToken[j];
						}
						if(SecondToken[j] != NULL){
							Aliased[i][j] = SecondToken[j];
						}
					}
